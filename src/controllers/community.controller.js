...
const createBroadcast = async (req, res) => {
  try {
    ...
    const result = await communityRepo.createBroadcast(req.user.id, { message, sport_id, play_date, start_time, end_time, players_needed });
...
const requestToJoin = async (req, res) => {
  try {
    ...
    if (!broadcastId) return res.status(400).json({ success: false, message: 'broadcastId is required' });
    ...
    if (broadcastResult.rows.length > 0) {
      ...
      getIO().to(`user_${host_id}`).emit('new_join_request', { broadcastId, userId: req.user.id });
      if (fcm_token) {
        await notificationQueue.add('join-request-notification', { tokens: [fcm_token], payload: { title: 'New Join Request!', body: 'Someone requested to join your broadcast.', data: { type: 'join_request', broadcastId: String(broadcastId) } } });
      }
    }
    ...
    if (error.code === '23505') return res.status(409).json({ success: false, message: 'You have already requested to join this broadcast' });
    res.status(500).json({ success: false, message: 'Internal Server Error' });
  }
};
...
const acceptRequest = async (req, res) => {
  try {
    ...
    if (updateReq.rows.length === 0) return res.status(404).json({ success: false, message: 'Request not found or unauthorized' });
    ...
    } else { roomId = roomResult.rows[0].id; }
    ...
    if (userResult.rows.length > 0 && userResult.rows[0].fcm_token) {
      await notificationQueue.add('request-accepted-notification', { tokens: [userResult.rows[0].fcm_token], payload: { title: 'Request Accepted!', body: 'Your request to join the match was accepted.', data: { type: 'request_accepted', broadcastId: String(broadcast_id), roomId: String(roomId) } } });
    }
...
const getChatHistory = async (req, res) => {
  try {
    ...
    if (participantCheck.rows.length === 0) return res.status(403).json({ success: false, message: 'Access denied to this chat room' });
...
const getRoomByBroadcastId = async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) return res.status(404).json({ success: false, message: 'Room not found or you are not a participant' });
...
module.exports = { createBroadcast, getFeed, getMyBroadcasts, requestToJoin, getRequests, acceptRequest, getChatHistory, getMyChats, getRoomByBroadcastId };
