...
const updateProfile = async (req, res) => {
  ...
  try {
    const result = await userRepo.updateUserProfileSimple(req.user.id, { name, email, phone });
    if (result.rows.length === 0) return res.status(404).json({ success: false, message: 'User not found' });
...
const getTurfSlots = async (req, res) => {
  ...
  if (!date || !sport_id) return res.status(400).json({ success: false, message: 'date and sport_id query parameters are required' });
  try {
    const slots = await bookingService.getTurfSlots(req.params.id, { date, sport_id });
...
const createBooking = async (req, res) => {
  ...
  if (!turf_id || !sport_id || !date) return res.status(400).json({ success: false, message: 'turf_id, sport_id and date are required' });
  ...
  try {
    const data = await bookingService.createBooking(req.user.id, { turf_id, sport_id, date, time_slots, is_full_day });
    return res.status(201).json({ success: true, message: 'Payment pending. Proceed to pay.', data });
  } catch (err) {
...
const cancelBooking = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Booking cancelled successfully. Time slot has been freed up.', data });
  } catch (err) {
...
const verifyPayment = async (req, res) => {
  ...
  try {
    const data = await bookingService.verifyPayment(req.user.id, { razorpay_order_id, razorpay_payment_id, razorpay_signature });
    return res.status(200).json({ success: true, message: 'Payment verified successfully. Booking CONFIRMED!', data });
  } catch (err) {
...
const rescheduleBooking = async (req, res) => {
  ...
  if (!date || !start_time || !end_time) return res.status(400).json({ success: false, message: 'date, start_time, and end_time are required' });
  try {
    const data = await bookingService.rescheduleBooking(req.user.id, req.params.id, { date, start_time, end_time });
    return res.status(200).json({ success: true, message: 'Booking rescheduled successfully', data });
  } catch (err) {
...
const changePassword = async (req, res) => {
  ...
  if (!current_password || !new_password) return res.status(400).json({ success: false, message: 'Current password and new password are required' });
  try {
...
module.exports = {
  getActiveTurfs, getProfile, updateProfile, getTurfSlots, createBooking, cancelBooking,
  verifyPayment, getCustomerBookings, rescheduleBooking, getTurfFeedbacks,
  getNotifications, markNotificationRead, deleteNotification, changePassword, getActivePromos
};
