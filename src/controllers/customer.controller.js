...
const getProfile = async (req, res) => {
	...
	try {
		const result = await db.query('SELECT id, name, email, phone, role, status, created_at FROM users WHERE id = $1',
			[userId]);
...
const updateProfile = async (req, res) => {
	...
	try {
		const result = await db.query(`UPDATE users SET name = COALESCE($1, name), email = COALESCE($2, email), phone = COALESCE($3, phone), updated_at = CURRENT_TIMESTAMP WHERE id = $4 RETURNING id, name, email, phone, created_at, updated_at`,
...
const getTurfSlots = async (req, res) => {
	...
	try {
		...
		if (turfSportCheck.rows.length === 0) {
			return res.status(400).json({
				success: false,
				message: 'The selected sport is not available at this turf'
			});
		}
		// 2. Get existing CONFIRMED bookings for this turf on this date and sport
		const bookingResult = await db.query(`SELECT start_time, end_time FROM bookings WHERE turf_id = $1 AND booking_date = $2 AND sport_id = $3 AND status = 'CONFIRMED'`,
			[id, date, sport_id]);
...
const createBooking = async (req, res) => {
	...
	try {
		...
		const conflictResult = await client.query(`SELECT id FROM bookings WHERE turf_id = $1 AND sport_id = $2 AND booking_date = $3 AND status = 'CONFIRMED' AND start_time = ANY($4)`,
			[turf_id, sport_id, date, slotStarts]);
		if (conflictResult.rows.length > 0) {
			await client.query('ROLLBACK');
			return res.status(409).json({
				success: false,
				message: 'One or more selected slots have already been booked by someone else!'
			});
		}
		...
		for (const slot of requestedSlots) {
			const bookingRes = await client.query(`INSERT INTO bookings (turf_id, sport_id, customer_id, booking_date, start_time, end_time, status, total_price, razorpay_order_id)
...
const cancelBooking = async (req, res) => {
	...
	const userId = req.user.id;
	try {
		const result = await db.query(`UPDATE bookings SET status = 'CANCELLED', updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND customer_id = $2 AND status = 'CONFIRMED' RETURNING *`,
			[id, userId]);
		if (result.rows.length === 0) {
			return res.status(404).json({
				success: false,
...
const verifyPayment = async (req, res) => {
	...
	try {
		...
		const updateResult = await db.query(`UPDATE bookings 
       ...
       RETURNING *`,
		...
		const receiptResult = await db.query(`SELECT b.*, t.name as turf_name, t.address, t.city, t.latitude, t.longitude
       FROM bookings b
...
const rescheduleBooking = async (req, res) => {
	...
	if (newBookingDateLocal < new Date()) {
		return res.status(400).json({
			success: false,
			message: 'Cannot reschedule to a time slot in the past'
		});
	}
	const client = await db.pool.connect();
	try {
		...
		if (originalBookingDate <= twoHoursFromNow) {
			...
			return res.status(400).json({
				success: false,
				message: 'Rescheduling is only allowed at least 2 hours before the match starts'
			});
		}
		// 2. Check for conflicts
		const conflictResult = await client.query(`SELECT id FROM bookings 
...
const markNotificationRead = async (req, res) => {
	...
	try {
		const result = await db.query('UPDATE notifications SET is_read = TRUE WHERE id = $1 AND user_id = $2 RETURNING *',
			[id, userId]);
		if (result.rows.length === 0) {
			return res.status(404).json({
				success: false,
