...
const replyToQuery = async (req, res) => {
	...
	if (!admin_reply) return res.status(400).json({
		success: false,
...
const notifyNearbyUsers = async (req, res) => {
	...
	if (!turf_id || !title || !body) return res.status(400).json({
		success: false,
		message: 'Missing required fields (turf_id, title, body)'
	});
	if (parsedRadius <= 0 && !hasSpecificCustomers) return res.status(400).json({
		success: false,
	...
	try {
		...
		if (turfResult.rows.length === 0) return res.status(404).json({
			success: false,
			message: 'Turf not found'
		});
		const turf = turfResult.rows[0];
		if ((!turf.latitude || !turf.longitude) && radius_km) return res.status(400).json({
			success: false,
		...
		if (parsedRadius > 0) {
			...
			queryParams.push(turf.longitude, turf.latitude, parsedRadius * 1000);
			paramIndex += 3;
		}
		...
		if (eligibleUsers.length === 0) return res.status(200).json({
			success: true,
		...
		await notificationRepo.insertCampaign({
			turfId: turf_id,
			title,
			body,
			radius_km,
			usersCount: eligibleUsers.length,
			targetedNames: targetedNamesStr
		});
...
const getNearbyCustomers = async (req, res) => {
	...
	if (!radius_km) return res.status(400).json({
		success: false,
	...
	try {
		...
		const turf = turfResult.rows[0];
		if (!turf.latitude || !turf.longitude) return res.status(400).json({
			success: false,
			message: 'Turf coordinates not set'
		});
		const result = await db.query(`SELECT id, name, email, phone, fcm_token IS NOT NULL as has_app FROM users WHERE role = 'CUSTOMER' AND ST_DWithin(location, ST_MakePoint($1, $2)::geography, $3) ORDER BY name ASC`,
...
const notifySingleUser = async (req, res) => {
	...
	try {
		...
		if (!user.fcm_token) return res.status(400).json({
			success: false,
...
const toggleFeaturedTurf = async (req, res) => {
	...
	if (is_featured === undefined) return res.status(400).json({
		success: false,
	...
	try {
		...
		return res.status(200).json({
			...
			data: {
				...
				successfulPayments: parseInt(bookingsStatsResult.rows[0].total_bookings),
